...
const REWARD_MULTIPLIERS: Record<number, number> = { 1: 1.0, 2: 2.0, 3: 3.0, 4: 4.0, 5: 5.0 };

export function BloomBoost() {
  const { balance, flowers } = useBloomStore();
  const { campaignCount, userReward: contractReward, claimCampaign, createCampaign } = useBloomBoost();
  ...
  const highestFlowerLevel = Math.max(...flowers.filter((f) => f.isUnlocked).map((f) => f.level), 1);
  const rewardMultiplier = REWARD_MULTIPLIERS[highestFlowerLevel] || 1;
  const calculatedReward = BASE_REWARD * rewardMultiplier;
  const finalReward = contractReward && contractReward > 0 ? contractReward : calculatedReward;

  const campaignIds = Array.from({ length: campaignCount }, (_, i) => i + 1).reverse();

  const handleViewDetails = (campaign: Campaign) => setSelectedCampaign(campaign);
  ...
  const handleClaimReward = async () => {
    ...
    try {
      ...
      toast.success('Rewards claimed! 🎉', { description: 'Campaign reward claimed successfully.' });
  ...
  const handleCreateCampaign = async (campaign: Campaign, payWithBloom: boolean) => {
    try {
      setCreating(true);
      await createCampaign(Number(campaign.totalPool), payWithBloom);
      toast.success('Bloom Boost created! 🚀', { description: 'Your campaign is now live.' });
    ...
    <div className="min-h-screen bloom-gradient-bg pb-24">
      ...
      <main className="px-4 py-6 max-w-md mx-auto space-y-6">
        ...
        <section>
          ...
          <div className="space-y-4">
            {campaignIds.map((id) => (
              <CampaignCard key={id} id={id} userReward={finalReward} onViewDetails={handleViewDetails} />
            ))}
          </div>
        </section>
      </main>
      ...
      {selectedCampaign && (
        <div className="fixed inset-0 z-50 flex items-end justify-center">
          <div className="absolute inset-0 bg-black/50" onClick={() => setSelectedCampaign(null)} />
          ...
          <div className="relative w-full max-w-md bg-card rounded-t-3xl max-h-[90vh] flex flex-col overflow-hidden animate-bloom">
            <div className="flex items-center justify-between p-6 border-b border-border shrink-0">
              <div>
                <h2 className="text-lg font-bold text-foreground">Campaign Tasks</h2>
                <p className="text-sm text-muted-foreground">Complete all tasks to claim reward</p>
              </div>
              <button
            ...
            <div className="flex-1 overflow-y-auto px-6 py-4 pb-32">
              <div className="mb-4">
                <p className="text-sm text-foreground leading-relaxed">{selectedCampaign.castText}</p>
              </div>
              ...
              <div className="space-y-4">
                {selectedCampaign.tasks.map((task) => {
                  ...
                  return (
                    <div key={task.type} className="p-4 rounded-xl border border-border bg-secondary/40">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="font-medium text-foreground capitalize">{task.type}</p>
                            <p className="text-xs text-muted-foreground">Complete this action</p>
                          </div>
                        </div>
                        ...
                        {!task.completed ? (
                          <div className="flex gap-2">
                            <button
                              onClick={() =>
                                handleTaskAction(selectedCampaign.id, task.type)
                              }
                            ...
                            <button
                              onClick={() =>
                                handleVerifyTask(selectedCampaign.id, task.type)
                              }
            ...
            <div className="sticky bottom-0 left-0 right-0 p-6 bg-card border-t border-border z-20">
              <button
                ...
                disabled={!selectedCampaign.tasks.every((t) => t.completed)}
                className={cn(
                  'w-full py-4 rounded-xl font-bold text-lg transition-all',
                  selectedCampaign.tasks.every((t) => t.completed)
...
function CampaignCard({ id, userReward, onViewDetails }: CampaignCardProps) {
  ...
  const mappedCampaign: Campaign = {
    ...
    creatorAvatar: '[api.dicebear.com](https://api.dicebear.com/7.x/avataaars/svg?seed=bloom)',
    castText: c.castText || 'Complete social tasks and earn rewards 🌸',
    castLink: c.castLink || '[warpcast.com](https://warpcast.com)',
    ...
    >
      <div className="flex items-center gap-3 mb-3">
        ...
        <div className="flex-1">
          <p className="font-medium text-foreground">@{mappedCampaign.creatorUsername}</p>
          <p className="text-xs text-muted-foreground">{mappedCampaign.participants} participants</p>
        </div>
      ...
      <p className="text-sm text-foreground mb-3 line-clamp-2">{mappedCampaign.castText}</p>
          ...
          return (
            ...
            >
              <Icon
                className={cn(
                  'w-4 h-4',
                  task.completed ? 'text-bloom-green' : config.color
                )}
              />
            </div>
          );
      ...
      <div className="flex items-center justify-between pt-3 border-t border-border">
        <span className="text-sm text-muted-foreground">
          Pool:{' '}
          <span className="font-bold text-foreground">{poolFormatted}</span>
        </span>
        {Boolean(hasClaimed) ? (
          <span className="text-sm font-medium text-bloom-green">Claimed</span>
        ) : (
...
interface CreateBoostModalProps {
  onClose: () => void;
  onCreate: (campaign: Campaign, payWithBloom: boolean) => void;
  balance: number;
  creating: boolean;
}

function CreateBoostModal({ onClose, onCreate, balance, creating }: CreateBoostModalProps) {
  ...
  const [tasks, setTasks] = useState({ like: true, recast: true, follow: true, reply: true });
  ...
  const handleLaunch = async () => {
    ...
    const newCampaign: Campaign = {
      ...
      creatorUsername: 'you',
      creatorAvatar: '[api.dicebear.com](https://api.dicebear.com/7.x/avataaars/svg?seed=you)',
      castText: castLink || 'My awesome Bloom Boost campaign! 🌸',
      castLink: castLink || '[warpcast.com](https://warpcast.com/you/0x789)',
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center">
      ...
      <div className="relative w-full max-w-md bg-card rounded-t-3xl animate-bloom max-h-[85vh] flex flex-col overflow-hidden">
        <div className="flex items-center justify-between p-6 border-b border-border shrink-0">
          <div className="flex items-center gap-2">
        ...
        {/* Reduced vertical padding and scroll height */}
        <div className="flex-1 overflow-y-auto px-6 py-3 space-y-5 pb-20">
          <div className="p-3 rounded-xl bg-secondary/50">
            <p className="text-sm text-muted-foreground">
              Your Balance:{' '}
              <span className="font-bold text-foreground">{formatBloom(balance)} BLOOM</span>
            </p>
          </div>

          <div>
            <label className="text-sm text-muted-foreground mb-2 block">Pay With</label>
          ...
          <div>
            <label className="text-sm text-muted-foreground mb-1 block">Cast Link</label>
            <input
              type="text"
              value={castLink}
              onChange={(e) => setCastLink(e.target.value)}
              placeholder="[warpcast.com](https://warpcast.com/)"
          ...
          <div>
            <label className="text-sm text-muted-foreground mb-1 block">Campaign Budget</label>
            <input
              type="number"
              value={budget}
          ...
          <div>
            <label className="text-sm text-muted-foreground mb-2 block">Tasks</label>
            <div className="space-y-2">
              {Object.entries(tasks).map(([task, enabled]) => (
                <button
                  key={task}
                  type="button"
                  onClick={() =>
                ...
                >
                  <span className="capitalize text-foreground">{task}</span>
                  {enabled && <Check className="w-4 h-4 text-bloom-pink" />}
                </button>
              ))}
            </div>
          </div>
        ...
        <div className="sticky bottom-0 left-0 right-0 p-5 bg-card border-t border-border z-20">
          <button
